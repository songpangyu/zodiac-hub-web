...
export default function ProfilePage() {
  ...
  const fetchUserData = async () => {
    try {
      ...
      setBookmarks(
        bookmarksSnap.docs.map((doc) => ({ id: doc.id, ...doc.data() }))
      );
  ...
  const handleDeleteHistory = async (historyId) => {
    try {
      await deleteDoc(
        doc(db, 'users', user.uid, 'compatibility_history', historyId)
      );
      setCompatibilityHistory(
        compatibilityHistory.filter((h) => h.id !== historyId)
      );
    } catch (error) {
              ...
              {profile?.zodiacSign && (
                <div className="flex items-center justify-center md:justify-start gap-2 mt-2">
                  <span className="text-2xl">
                    {
                      zodiacSigns.find((s) => s.name === profile.zodiacSign)
                        ?.symbol
                    }
                  </span>
                    ...
                    <div className="flex items-center gap-4">
                      <div className="flex items-center gap-2">
                        <span className="text-3xl">
                          {
                            zodiacSigns.find((s) => s.name === item.sign1)
                              ?.symbol
                          }
                        </span>
                        ...
                        <span className="text-3xl">
                          {
                            zodiacSigns.find((s) => s.name === item.sign2)
                              ?.symbol
                          }
                        </span>
                      </div>
