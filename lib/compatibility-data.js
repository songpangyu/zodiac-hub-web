...
// [NEW] Helper function to convert string to a consistent number
// This ensures that "Aries" + "Taurus" always generates the same 'random' variance
...
// [UPDATED] Modified to use deterministic hash instead of Math.random()
function generateAspects(sign1, sign2, score) {
    // Create a seed from the sorted sign names (so order doesn't matter: Aries+Leo = Leo+Aries)
    const seedString = [sign1, sign2].sort().join("");
    const hash = stringToHash(seedString);

    // [UPDATED] Use hash modulo to determine fluctuation instead of Math.random
